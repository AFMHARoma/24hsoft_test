...
class DBGateway:
    ...
    async def insert_to_base(self, name, data: list) -> None:
        query = f'''
        ...
        async with self.pg_pool.connection() as conn:
            async with conn.cursor() as cur:
                ...
                await cur.execute(f"DELETE FROM {name} WHERE match_start_time < CURRENT_TIMESTAMP")

    async def fetch(self, query: str, data: list | tuple = None) -> list:
        async with self.pg_pool.connection() as conn:
            async with conn.cursor() as cur:
...
class Parser:
        ...
        while True:
            try:
                async with aiohttp.ClientSession(headers=headers) as session:
                    async with session.get(base_url + endpoint, params=req_params) as resp:
                        if resp.status == 200:
    ...
    @staticmethod
    def format_for_base(prefix, data: list) -> list:
        formatted_data = []
            ...
            for match in tournament['upc_matches']:
                formatted_data.append(
                    [
                        ...
                        datetime.datetime.strptime(tournament['date' if prefix == 'cs2' else 'start_date'],
                                                   '%Y-%m-%dT%H:%M:%SZ'),
...
class EsportsBattleParser(Parser):
    ...
    def __init__(self, config: dict, db: DBGateway):
        self.config = config
        self.db = db
        self.most_far_tournaments_datetime_dict = {}

    @staticmethod
    def filter_events(acceptable_ids: list, data: list) -> list:
        for item in data[:]:
            if item["status_id"] not in acceptable_ids:
                data.remove(item)
        return data

    async def set_most_far_tournament_datetime_by_sport(self, prefix) -> None:
    ...
    async def get_valid_status_codes(self, url: str) -> dict[str, list]:
        ...
        codes_response = await self.do_request(url, self.status_codes_endpoint)

        for event_type, codes_list in codes_response.items():
            for code in codes_list:
                if code["token_international"] in self.config.get(f"filter_status_code_{event_type}", []):
                    acceptable_ids[event_type].append(code["id"])

        print(acceptable_ids)
        return acceptable_ids

    async def get_filtered_tournaments(self, url: str, prefix: str, status_codes=None, page=1) -> list:
        ...
        params = {
            "page": page,
            "dateFrom": max(self.most_far_tournaments_datetime_dict.get(prefix, date_from), date_from).strftime(
                "%Y/%m/%d %H:%M"),
            "dateTo": date_to.strftime("%Y/%m/%d %H:%M"),
        }
        print(prefix, params)

        tournaments_data: dict = await self.do_request(url, self.tournaments_endpoint, params)
        filtered_tournaments_list.extend(tournaments_data["tournaments"])
        print(tournaments_data)
        if page == 1:
            parse_tasks = []
            for item in range(2, tournaments_data["totalPages"] + 1):
                parse_tasks.append(
                    asyncio.create_task(self.get_filtered_tournaments(url, prefix, page=item))
                )
            parse_tasks_res = await asyncio.gather(*parse_tasks)
            print(parse_tasks_res)
            filtered_tournaments_list.extend(
                [
                    item
                    for sublust in parse_tasks_res
                    for item in self.filter_events(status_codes, sublust)
                ]
            )
            if filtered_tournaments_list:
                ...
                self.most_far_tournaments_datetime_dict[prefix] = datetime.datetime.strptime(
                    filtered_tournaments_list[0][date_key], '%Y-%m-%dT%H:%M:%SZ')
        return filtered_tournaments_list

    async def get_filtered_matches(self, url: str, tournaments_list: list, status_codes: list) -> list:
        parse_tasks = []
        ...
        parse_tasks_res = await asyncio.gather(*parse_tasks)
        for q, item in enumerate(parse_tasks_res):
            tournaments_list[q]['upc_matches'] = self.filter_events(status_codes, item)
        return tournaments_list

    @async_logger
    async def parse_once(self, target_url, prefix, status_codes):
        tournaments = await self.get_filtered_tournaments(
            target_url, prefix, status_codes["tournament"]
        )
        upcoming_matches = await self.get_filtered_matches(
            target_url, tournaments, status_codes["match"]
        )
    ...
    async def start_parse(self) -> None:
        ...
        async with asyncio.TaskGroup() as gr:
            for item in self.config.get('sports_to_parse', []):
                if item in self.available_sports:
                    logging.info(f'Discovered sport: {item}')
                    gr.create_task(self.parse_in_loop(item))
